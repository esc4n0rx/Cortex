// components/upload-cadastro-card.tsx
"use client"
...
import { Upload, CheckCircle, AlertTriangle, FileSpreadsheet, X, Info, Database } from "lucide-react"
import { useUpload } from "@/hooks/use-upload"

interface UploadCadastroCardProps {
  onUploadSuccess?: () => void
}

export function UploadCadastroCard({ onUploadSuccess }: UploadCadastroCardProps) {
  ...
  const handleUpload = async () => {
    ...
    const result = await uploadFile(selectedFile, 'cadastro')
    if (result) {
   ...
   <Card className="rounded-2xl">
     <CardHeader>
       <CardTitle className="flex items-center gap-2">
         ...
         Upload de Cadastro de Itens
       </CardTitle>
       <CardDescription>
         Faça upload da planilha com cadastro de itens. Os dados anteriores serão substituídos.
       </CardDescription>
     </CardHeader>
     <CardContent className="space-y-4">
       ...
       <Alert variant="destructive">
         ...
         <AlertDescription>
           <strong>Atenção:</strong> Este upload irá remover todos os itens cadastrados anteriormente e substituí-los pelos dados da planilha.
         </AlertDescription>
       </Alert>
       ...
       {isUploading && (
         <div className="space-y-2">
           ...
           <div className="text-xs text-muted-foreground">
             Processando em lotes de 1000 registros
           </div>
         </div>
       )}
         ...
         <AlertDescription>
           <div className="space-y-2">
             ...
             <div className="text-xs mt-2">
               ...
               <div>• Material deve ser único (código do produto)</div>
               <div>• Quantidade por caixa deve ser maior que zero</div>
               <div>• Contador é opcional (padrão: 0)</div>
               <div>• Formato aceito: .xlsx, .xls, .csv</div>
               <div>• Processamento em lotes de 1000 registros</div>
             </div>
           </div>
         </AlertDescription>
       </Alert>
